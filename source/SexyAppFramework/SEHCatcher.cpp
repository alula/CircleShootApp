...
struct 
{
   ...
   { SIGFPE,     "Float Exception" },
   { SIGHUP,     "Hangup detected" },
   { SIGINT,     "Ctrl+C Exit" },
   { SIGILL,     "Illegal Instruction" },
   { SIGSEGV,    "Segmentaton Fault" },
...
{	
    ...
    sigaction(SIGABRT, &action, &mPreviousAction);
    sigaction(SIGSEGV, &action, &mPreviousAction);
#endif
}
...
#elif defined(_POSIX)
	sigaction(SIGSEGV, &mPreviousAction, NULL);
#endif
}

#if defined(_POSIX)
void SEHCatcher::HandleSigaction(int nSignum, siginfo_t *si, void *vcontext)
{
	if (mApp != NULL)
		mApp->SEHOccured();
	...
	{
        if (gMsgTable[i].dwExceptionCode == nSignum)
		{
	...
	{
#if defined(__LP64__)
		sprintf(aBuffer, "Exception: %s (code 0x%x) at address %016lX in thread %X\r\n",
				szName, nSignum, context->uc_mcontext.fault_address, getpid());
#else
	...
	{
#if defined(__LP64__)
		sprintf(aBuffer, "Unknown exception: (code 0x%x) at address %016lX in thread %X\r\n",
				nSignum, context->uc_mcontext.fault_address, getpid());
#else
