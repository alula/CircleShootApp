...
SexyAppBase::~SexyAppBase()
{
	...
	if (!showedMsgBox && Sexy::gD3DInterfacePreDrawError && !IsScreenSaver())
	{
		int aResult = MessageBox(NULL, 
...
void SexyAppBase::Redraw(Rect* theClipRect)
{
	...
	{
		Sexy::gD3DInterfacePreDrawError = false; // this predraw error happens naturally when ddraw is failing
		if (!gIsFailing)
		{
...
bool SexyAppBase::ShouldCheckForUpdate()
{
	return false;
}

void SexyAppBase::DoConfirmCheckForUpdatesDialog()
{
	DoDialog(1, true, _S("Updates"),
		_S("Do you want to check for updated to Zuma?\
  New versions may offer new features and bug fixes.\
  This requires an active Internet connection."), _S(""), Dialog::BUTTONS_YES_NO);
}

void SexyAppBase::UpdateCheckQueried()
{
